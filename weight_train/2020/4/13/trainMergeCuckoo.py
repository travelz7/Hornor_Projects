...
def train(test_use, i, j):
    ...
    # z = x1 * w1 * 0.3333 + x2 * w2 * 0.2778 + x3 * w3 * 0.1667 + x4 * \
    # w4 * 0.1111 + x5 * w5 * 0.0556 + x6 * w6 * 0.0556 + Bias
    real_output = np.tanh(z)
    err = aim_output - real_output
    # err = 0.5*((aim_output - real_output)*(aim_output - real_output))
    # print('误差：\n', err)
    w1 = w1 + eta*err*x1
    w2 = w2 + eta*err*x2
    w3 = w3 + eta*err*x3
    ...
    print('z：\n', z)
    real_output = np.tanh(z)
    # print('tahn后的z,就是real_output', real_output)
    err = aim_output - real_output
...
empi_unique = pd.read_excel(
        io=r'D:\Python\Projects\weight_train\data\more_unique.xlsx'
                                )
empi_unique = empi_unique[0:30400]
# 身份证集合
...
def sample_merging():
    # 复制一个测试集
    # test_use = empi_unique[30400: 38175]
